...
BG_UNCONFINED_FLAG[?] BG_Sy[f]  BG_Kz[f]  BG_b[f]
BG_DP_FLAG[?] BG_MAT_Ss[f] BG_MAT_LAM[f] BG_MAT_DIFF_IDX[i] BG_MAT_DM[f] BG_MAT_ND[i]
...
An example of an elliptical (similar to circular) element input file for four elements is listed
...
EL_MAT_Ss[f] * NE
EL_MAT_LAM[f] * NE
EL_MAT_DIFF_IDX[i] * NE
EL_MAT_DM[f] * NE
EL_MAT_ND[i] * NE
EL_SKIN[f] * NE
EL_AREA_SOURCE[f] * NE
EL_BDRY_SOURCE[f] * NE
EL_AREA_TFLAG[i] EL_AREA_TPAR1[f] EL_AREA_TPAR2[f]
[NE rows like this]
EL_BDRY_TFLAG[i] EL_BDRY_TPAR1[f] EL_BDRY_TPAR2[f]
[NE rows like this]
\end{verbatim}
\end{minipage}
\newpage
For elements, most items in the input take up a single row, and are repeated \texttt{NE} (number of ellipses) times (specified next to the file name in the main input file). If only one value is listed, that one value will be used for all the elements. Either the correct number of values can be read, or only one (if more are listed than are needed, the later ones are ignored).

The items related to time behavior are at the end of the file, and they are one element per row, and the ``repeating'' rule for reading inputs mentioned above does not apply (there must be one entry for each element).
