...
\begin{verbatim}
CALC[?] PARTICLE[?] CONTOUR[?] TIMEDERIV[?] OUTPUT_TYPE[i]
OUTPUT_FN[s] 
BG_POROSITY[f] BG_K[f]  BG_Ss[f]   
BG_LEAK_FLAG[i]  BG_AQT_K[f] BG_AQT_Ss[f]  BG_AQT_b[f]
UNCONFINED_FLAG[?] BG_Sy[f]  BG_Ks[f]  BG_AQF_b[f]
DUALPORO_FLAG[?] BG_MATRIX_Ss[f] BG_LAMBDA[f]
...
Some error and consistency checks are made at run-time, when the input file is read (i.e., physical variables are not negative or zero, integer flags have valid values). When a simulation is run, a \texttt{.echo} file is written that echos the input, with a terse explanation of what each variable means. This is useful to check, to make sure the 
