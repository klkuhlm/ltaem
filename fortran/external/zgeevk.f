      SUBROUTINE ZGEEVK( JOBVL, JOBVR, N, A, LDA, W, VL, LDVL, VR, LDVR,
...
c$$$            DO 10 K = 1, N
c$$$               RWORK( IRWORK+K-1 ) = DBLE( VL( K, I ) )**2 +
c$$$     $                               DIMAG( VL( K, I ) )**2
c$$$   10       CONTINUE
c$$$            K = IDAMAX( N, RWORK( IRWORK ), 1 )
c$$$            TMP = DCONJG( VL( K, I ) ) / SQRT( RWORK( IRWORK+K-1 ) )
c$$$            CALL ZSCAL( N, TMP, VL( 1, I ), 1 )
c$$$            VL( K, I ) = DCMPLX( DBLE( VL( K, I ) ), ZERO )
   20    CONTINUE
      END IF
*
      IF( WANTVR ) THEN
