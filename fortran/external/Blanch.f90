  ...
  SUBROUTINE MTU12_Blanch(KF,KC,N,Q,X,F1R,D1R,F2R,D2R,KM)
    ...
    IF ( (KC==1) .OR. (KC==3)) THEN
       ...
       DO K=1,KM
          ...
          ELSE IF (KD.EQ.2.OR.KD.EQ.3) THEN
             D1R=D1R+(-1)**(IC+K)*FG(K)*(C2*(BJ1(K-1)*DJ2(K)+(-1)**KD*BJ1(K)*DJ2(K-1))-&
    ...
    IF ( (KC==2) .OR. (KC==3)) THEN
       ...
       DO K=1,KM
          ...
          ELSE IF (KD.EQ.2.OR.KD.EQ.3) THEN
             D2R=D2R+(-1)**(IC+K)*FG(K)*(C2*(BJ1(K-1)*DY2(K)+(-1)**KD*BJ1(K)*DY2(K-1))-&
                  &C1*(DJ1(K-1)*BY2(K)+(-1)**KD*DJ1(K)*BY2(K-1)))
          ELSE
             D2R=D2R+(-1)**(IC+K)*FG(K)*(C2*(BJ1(K-1)*DY2(K+1)-BJ1(K+1)*DY2(K-1))-&
